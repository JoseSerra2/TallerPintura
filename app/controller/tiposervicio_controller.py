...
@router.get("/pintura/GET/tiposervicios", response_model=List[TipoServicioSchema])
def listar_tiposervicios(db: Session = Depends(get_db)):
    return get_tiposervicios(db)


@router.post("/pintura/POST/tiposervicios", response_model=TipoServicioSchema)
...
@router.put("/pintura/PUT/tiposervicios/{idTipoServicio}", response_model=TipoServicioSchema)
