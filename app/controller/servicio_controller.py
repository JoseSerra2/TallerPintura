...
@router.get("/pintura/GET/servicios", response_model=List[ServicioSchema])
def listar_servicios(db: Session = Depends(get_db)):
    return get_servicios(db)


@router.post("/pintura/POST/servicios", response_model=ServicioSchema)
def crear_servicio(servicio: ServicioCreate, db: Session = Depends(get_db)):
    return create_servicio(db, servicio)


@router.put("/pintura/PUT/servicios/{idServicio}", response_model=ServicioSchema)
