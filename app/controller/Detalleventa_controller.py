...
@router.post("/pintura/POST/detalleventas", response_model=DetalleVentaResponse)
def crear_un_detalle(detalle: DetalleVentaCreate, db: Session = Depends(get_db)):
    return crear_detalle(db, detalle)

@router.get("/pintura/GET/detalleventas/{idVenta}", response_model=List[DetalleVentaResponse])
...
@router.put("/pintura/PUT/detalleventas/{idDetalleVenta}", response_model=DetalleVentaResponse)
