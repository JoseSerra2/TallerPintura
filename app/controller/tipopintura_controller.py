...
@router.get("/GET/tipopinturas", response_model=List[TipoPinturaSchema])
def listar_tipopinturas(db: Session = Depends(get_db)):
    return get_tipopinturas(db)


@router.post("/POST/tipopinturas", response_model=TipoPinturaSchema)
def crear_tipopintura(tipopintura: TipoPinturaCreate, db: Session = Depends(get_db)):
    return create_tipopintura(db, tipopintura)


@router.put("/PUT/tipopinturas/{idTipoPintura}", response_model=TipoPinturaSchema)
